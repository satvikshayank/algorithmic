...
def main():
    ...
    if choice == "Home":
        ...
        if st.sidebar.checkbox("Login") or (password == '1234' and username == 'shayank'):
            # if password=='1234' and username=='sayank':
            create_usertable()
            result = 0
...
def mainfunc():
    ...
    app_mode = st.selectbox("Select the app mode", ["Home", "Data Analysis", "Prediction"])
...
def data_analysis():
    ...
    if show == "Graphs":
        ...
        fig = go.Figure(data=[go.Candlestick(x=df1.index,
                                             ...
                                             close=df1['Close'],
                                             name='Market Data'),
                              go.Scatter(x=list(df1.index), y=list(df1.MA), line=dict(color='blue', width=2),
                                         name='Moving Average')])
...
def prediction():
    ...
    pred = st.sidebar.radio("Regression Type", ["Tree Prediction", "Linear Regression","SVR Prediction","RBF Prediction","LSTM"])
    ...
    if pred == "Linear Regression":
        ...
        group_labels = ['Close', 'Vclose', 'Vpredictions']
